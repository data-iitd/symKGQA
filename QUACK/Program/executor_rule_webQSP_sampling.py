...
def processKB(kb):
    ...
    mapping2 = pd.read_csv('path of entity id to name mapping file1')
        ...
        for j in range(len(val['Questions'][i]['Parses'])):
            for k in range(len(val['Questions'][i]['Parses'][j]['Answers'])):
                try:
                    ...
                    answer.append(val['Questions'][i]['Parses'][j]['Answers'][k]['EntityName'])
...
def main():
    ...
    errorIdx = getErrorIndex(log_file)
